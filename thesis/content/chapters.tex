...
Reinforcement learning (RL) is becoming more popular in the last decade because it
...
they are not follow all of the above-mentioned factors and rely on the
technical analysis and covariances between the assets, which
do not helps so much in prediction and understanding the market and
quality of each asset in the portfolio, on top of that, increasing the
complexity of the model far more than is its worth.
...
\begin{itemize}
    ...
    and indexes such as S\&P 500, etc....
\end{itemize}

Using programming language Python and the open-source libraries such as numpy, pandas, stable-baselines3, gym, etc...
I will implement the RL agents and evaluate their performance on the dataset.

This thesis aims to discover if RL can learn optimal portfolio management strategies
that yields better results than the current state-of-the-art portfolio management
methods and will focus on the objectives mentioned above.
