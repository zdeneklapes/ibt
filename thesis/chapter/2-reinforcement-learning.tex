...
%    \end{definition}
    ...
    Both first-visit MC and every-visit MC converge to $v_\pi(s)$ as the number of visits (or first visits) to $s$ goes to infinity. In the case of first-visit MC, each return is an independent, identically distributed estimate of $v_\pi(s)$ with finite variance, and the sequence of averages of these estimates converges to their expected value by the law of large numbers. Each average is an unbiased estimate, and the standard deviation of its error decreases as $\frac{1}{\sqrt{n}}$, where $n$ is the number of returns averaged. Every-visit MC also converges quadratically to $v_\pi(s)$~\cite{sutton2018reinforcement}, see the \cref{alg:mc} in Appendices.
