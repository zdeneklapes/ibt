...
%    \end{wrapfigure}
    ...
    The Approximate Solution Methods use a function approximator $f_{\bm{\theta}}$ to approximate the action-value function \(q_\pi(s,a)\) or the state-value function \(v_\pi(s)\) as:
...
%
    ...
    \begin{equation}
        \begin{split}
            ...
            &\mspace{10mu}\vdots\\
            &=\sum_{s}\sum_{k=0}^{\infty}\mathbb{P}(s_0\rightarrow s,k,\pi_{\bm{\theta}})\sum_{a}\nabla\pi(a|s,\bm{\theta})q_\pi(s,a)
        \end{split}
    \end{equation}


    where $P(s \rightarrow s', k, \pi_{\bm{\theta}})$ is the probability of a state transition from $s$ to $s'$ in $k$ steps under policy $\pi(a | s, \theta)$. This probability thus serves as a weighting of the gradient of the return. Moreover, if $\eta(s) = \sum_{k=0}^{\infty} P(s \rightarrow s', k, \pi_{\theta})$ is defined, it can be further written:
    ...
    The gradient of \(J(\bm{\theta})\) is proportional to the probability sum of \(\sum_s \mu(s)\), which denotes the likelihood that the agent is in state \(s\), multiplied by the sum of weighted gradients of the probabilities for the selection of actions. The proportionality constant is represented as \(\sum_s \eta(s)\).
